...
describe("Testing 'clone' command", function () {
    describe("Clone by repository url", function () {
        it("Test folders and files exists", function (done) {
            ...
            ndev.cmdClone(["https://github.com/javanile/nodejs-skeleton"], function(log) {
                chai.assert.isDirectory(ndev.cwd + "/ndev_modules/nodejs-skeleton/");
                chai.assert.isFile(ndev.cwd + "/ndev_modules/nodejs-skeleton/LICENSE");
                done();
            });
        });
    });
